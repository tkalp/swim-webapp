    ...
    async def fetch(self, url: str, retry_count: int = 0) -> str:
        ...
        try:
            ...
            if proxy_config:
                ...
                logger.info(f"✓ Using Oxylabs proxy: {proxy_config['server']} (country: {proxy_config['country'].upper()})")
            else:
                logger.warning("⚠ No proxy configured - making direct request")
            ...
            logger.info(f"Executing curl for: {url}")
