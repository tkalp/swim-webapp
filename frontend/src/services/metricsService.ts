...
export async function getSquadDistancePerWeek(
  ...
  const { data: sessions, error: sErr } = await sQ
  if (sErr) {
    console.error('Error fetching sessions for distance:', sErr)
    throw sErr
  }
  ...
  if (workoutIds.length) {
    const { data: workouts, error: wErr } = await supabase
      ...
      .in('id', workoutIds)
    if (wErr) {
      console.error('Error fetching workout totals:', wErr)
      throw wErr
    }
...
export async function getSquadStrokeBreakdown(
  ...
  const { data: sessions, error: sErr } = await sQ
  if (sErr) {
    console.error('Error fetching sessions for stroke breakdown:', sErr)
    throw sErr
  }
  if (!sessions?.length) return []
  ...
  const { data: workouts, error: wErr } = await supabase
    ...
    .in('id', workoutIds)
  if (wErr) {
    console.error('Error fetching workout descriptions:', wErr)
    throw wErr
  }
...
export async function getSquadActivityBreakdown(
  ...
  const { data: sessions, error: sErr } = await sQ
  if (sErr) {
    console.error('Error fetching sessions for activity breakdown:', sErr)
    throw sErr
  }
  if (!sessions?.length) return []
