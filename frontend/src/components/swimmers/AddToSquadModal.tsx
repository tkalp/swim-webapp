  ...
  return (
    <Modal
      ...
      title={
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-text-primary">Add to Squad</h2>
            <p className="text-sm text-text-secondary mt-0.5">Choose a squad for this swimmer</p>
          </div>
        </div>
      }
      size="md"
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="relative overflow-hidden bg-linear-to-br from-primary/5 to-primary/10 rounded-2xl p-5 border border-primary/20">
          ...
          <div className="relative">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-lg text-text-primary mb-1">{swimmer.name}</h3>
        ...
        <div>
          <label className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-2">
            ...
            {loadingSquads && <Loader2 className="w-4 h-4 animate-spin text-primary" />}
          </label>
          
          {loadingSquads ? (
            <div className="text-center py-12">
              <Loader2 className="w-10 h-10 mx-auto mb-3 animate-spin text-primary" />
              <p className="text-text-secondary text-sm">Loading your squads...</p>
            </div>
          ) : squads.length === 0 ? (
            <div className="text-center py-12 bg-background-secondary/30 rounded-2xl border border-border/40">
              <div className="w-16 h-16 rounded-full bg-background-secondary flex items-center justify-center mx-auto mb-4">
                <Users className="w-8 h-8 text-text-muted" />
              </div>
              <p className="text-text-primary font-medium mb-1">No squads found</p>
              <p className="text-text-secondary text-sm">Create a squad first to add swimmers</p>
            </div>
          ) : (
                ...
                <button
                  ...
                  className={`group w-full text-left p-4 rounded-xl border-2 transition-all duration-200 ${
                    selectedSquadId === squad.id
                      ...
                      : 'bg-background-elevated border-border/40 hover:border-primary/50 hover:shadow-md'
                  }`}
                >
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
                        ...
                        <Users className={`w-5 h-5 transition-colors ${
                          selectedSquadId === squad.id ? 'text-primary' : 'text-text-muted group-hover:text-primary'
                        }`} />
                      </div>
                      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 px-5 py-3 text-text-secondary font-medium hover:text-text-primary hover:bg-background-secondary rounded-xl transition-all"
            disabled={loading}
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 px-5 py-3 bg-linear-to-r from-primary to-primary/90 text-white rounded-xl hover:shadow-lg hover:shadow-primary/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-none font-semibold flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
