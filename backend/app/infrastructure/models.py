...
class TrainingSessionPrePracticeNote(Base):
    ...
    equipment_needed: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class TrainingSessionPostPracticeNote(Base):
    ...
    technique_quality: Mapped[Optional[int]] = mapped_column(Integer)
    positivity: Mapped[Optional[int]] = mapped_column(Integer)
    what_went_well: Mapped[Optional[str]] = mapped_column(Text)
    areas_for_improvement: Mapped[Optional[str]] = mapped_column(Text)
    next_session_focus: Mapped[Optional[str]] = mapped_column(Text)
    individual_highlights: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
